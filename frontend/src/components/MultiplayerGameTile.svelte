<script>
	...
	$: $multiplayerGame.lobby.round, (toggleFlip = false);
	$: {
		if ($multiplayerGame.lobby.users.length > 0) {
			if ($multiplayerGame.lobby.users[userIndex].words[wordIndex].wordSubmitted) {
				toggleFlip = true;
			}
			if (
				$multiplayerGame.lobby.users[userIndex].isDone &&
				$multiplayerGame.lobby.users[userIndex].currentWord > 5
			) {
				toggleFlip = true;
			}
		}
...
<div class="flex items-center justify-center flipBox" style={`height: ${height}; width: ${width} `}>
	<div
		class="relative w-full h-full text-center flipBoxInner"
